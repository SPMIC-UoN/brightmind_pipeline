...
Usage()
{
  ...
  echo " "
  echo " "
  echo " --path <path>                      (Mandatory) BRC Pipeline output path"
  echo " --subject <subject id>             (Mandatory) Subject ID as set in BRC Pipeline"
  echo " --landmarks-file <file path>       (Mandatory) Text file containing the coordinates in mm of the nasion and preauricular points"
  ...
  echo " --output-path <path>               (Optional)  Path of the folder where the compressed directories with StimGuide files are written (Default: 'StimGuide' folder inside subject directory)"
